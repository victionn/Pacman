...
public class Pacman implements Controllable, PlayerPositionSubject {
    ...
    //Collects the ghost, and applies the ghost eaten multiplier
    public void collectGhost(Level level, Renderable renderable) {
        int pointsEarned = ((Collectable) renderable).getPoints() * (int) Math.pow(2, ghostMultiplier - 1);
        level.collectGhost(pointsEarned);
        ghostMultiplier += 1;
    }

    //Reset ghost eating multiplier
    public void resetMultiplier() {ghostMultiplier = 1;}
