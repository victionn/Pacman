...
public class LevelImpl implements Level {
    ...
    private void initLevel(LevelConfigurationReader levelConfigurationReader) {
        ...
        Map<GhostMode, Integer> ghostDurations = levelConfigurationReader.getGhostModeLengths();

        for (Ghost ghost : this.ghosts) {
            player.registerObserver(ghost);
            ghost.setStateDuration(ghostDurations);
            ghost.setStateSpeed(ghostSpeeds);
    ...
    public void collect(Collectable collectable) {
        int points = collectable.getPoints();
        this.points += points;
        notifyObserversWithScoreChange(collectable.getPoints());
        this.collectables.remove(collectable);
        //If power pellet is collected
        if (collectable instanceof PowerPellet) {
            //Call the power pellet to set all ghosts to frightened mode
            collectable.enableFrightenedMode(ghosts);
            //Reset the multiplier for eating ghosts
            ((Pacman)player).resetMultiplier();
        }
    }
